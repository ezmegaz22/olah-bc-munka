  ...
  return (
    <article className="p-3 lg:p-5 mb-5 bg-white border border-blue-600 rounded-md">
      <header className="lg:flex justify-between mb-4">
        <div className="mb-4 lg:mb-0">
          <p className="font-semibold">
            <span>Rendelés azonosítója: {order?._id} </span>
            {order?.orderStatus == "Feldolgozás alatt" ? (
      ...
      <div className="grid md:grid-cols-3 gap-2">
        <div>
          <p className="text-gray-400 mb-1">Felhasználó</p>
          <ul className="text-gray-600">
            <li>{order?.user?.name}</li>
            <li>Telefonszám: {order?.shippingInfo?.phoneNo}</li>
            <li>Email: {order?.user?.email}</li>
          </ul>
        </div>
        <div>
          <p className="text-gray-400 mb-1">Szállítási cím</p>
          <ul className="text-gray-600">
      ...
      <div class="my-8">
        <label class="block mb-3"> Rendelés állapotának frissítése </label>
        <div class="relative">
          <select
            ...
            required
          >
            {["Feldolgozás alatt", "Kiszállítás alatt", "Kiszállítva"].map(
              (status) => (
